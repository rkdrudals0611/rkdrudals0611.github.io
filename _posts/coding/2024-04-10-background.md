...
CSS 박스 모델은 웹 페이지를 디자인할 때 기본이 되는 중요한 개념이에요. 각각의 HTML 요소는 마치 상자처럼 취급되며, 이 상자는 여러 부분으로 구성되어 있어요. 이 구성 요소들은 웹 페이지의 레이아웃을 결정하는 데 큰 역할을 합니다.

## 01. width, height 

CSS에서 width와 height 속성은 웹 페이지의 요소들이 차지하는 가로 및 세로 공간을 정의하는 데 사용됩니다. 이러한 속성을 통해 웹 페이지의 레이아웃을 정밀하게 조정할 수 있습니다.

## 02. Padding 

 패딩은 콘텐츠와 요소 테두리 사이의 공간입니다. 요소 내부에 공간을 제공하여 콘텐츠와 테두리 사이의 거리를 제어할 수 있습니다.

 ## 03. Margin 

CSS에서 마진은 요소의 바깥쪽 여백을 설정하는 속성입니다. 이를 통해 요소와 요소 사이의 공간을 조절할 수 있어, 웹 페이지의 레이아웃을 디자인하는 데 중요한 역할을 합니다.

 ## 04. Border  

테두리는 요소의 패딩과 콘텐츠를 둘러쌉니다. 일반적으로 실선이지만 점선이나 점선, 이미지 등 다양한 방식으로 스타일을 지정할 수 있습니다.

 ## 05. outline  

CSS의 outline 속성은 요소의 외곽선을 설정하는 데 사용됩니다. 이 외곽선은 요소의 테두리(border) 바깥에 그려지며, 테두리와 달리 요소의 크기나 레이아웃에 영향을 주지 않습니다. 아웃라인은 주로 접근성을 향상시키기 위해 포커스가 요소에 있을 때 시각적으로 표시하는 용도로 사용됩니다.

 ## 06. box-sizing  

CSS의 box-sizing 속성은 요소의 크기 계산 방법을 결정합니다. 기본적으로 요소의 크기는 내용(content)의 크기만을 포함하지만, box-sizing 속성을 사용하면 테두리(border)와 내부 여백(padding)을 포함한 크기로 계산할 수 있습니다. 이 속성은 웹 페이지의 레이아웃을 더 쉽게 제어할 수 있게 해줍니다.

## 07. box-shadow  

CSS의 box-shadow 속성을 사용하면 요소에 그림자 효과를 추가할 수 있습니다. 이 속성은 웹 페이지의 요소에 깊이감을 주거나 시각적 관심을 끌기 위해 자주 사용됩니다. box-shadow는 요소의 프레임 바깥쪽에 그림자를 생성하며, 여러 값을 조합하여 다양한 그림자 효과를 만들 수 있습니다.

## 08. resize  

CSS의 resize 속성을 사용하면 사용자가 웹 페이지의 특정 요소의 크기를 조절할 수 있게 됩니다. 이 속성은 주로 텍스트 영역이나 다른 컨테이너 요소에 적용되어, 사용자가 인터페이스의 일부분을 자신의 필요에 맞게 조절할 수 있도록 합니다. 하지만 모든 요소에 적용될 수 있는 것은 아니며, 인라인 요소나 일부 블록 요소에는 적용되지 않습니다.

## 09. appearance  

CSS의 appearance 속성은 요소가 사용자의 운영 체제 테마에 기반한 플랫폼 네이티브 스타일링을 사용하여 표시되도록 합니다. 이 속성을 사용하면 플랫폼 특정 스타일링을 요소에 적용하거나 제거할 수 있습니다. 주로 폼 요소나 버튼 등에 기본적으로 적용된 스타일을 초기화하거나 변경할 때 유용하게 사용됩니다.